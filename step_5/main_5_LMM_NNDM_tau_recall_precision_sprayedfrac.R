# ============================================================
# LMM analysis for recall / precision / sprayed fraction vs. τ
# Extends main_4_LMM_NNDM_tau_recall_precision.R with a third
# panel showing sprayed_frac as a function of probability threshold τ.
...
# 2. Settings
# ------------------------------------------------------------
tau_grid <- c(0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90)
...
dfs <- lapply(names(alg_files), function(alg) {
  fp <- alg_files[[alg]]
  if (!file.exists(fp)) { warning("Missing file: ", fp); return(NULL) }
...
dat <- dat_raw %>%
  ...
  mutate(
    ...
    sprayed_frac = as.numeric(sprayed_frac)
  ) %>%
...
fit_one_tau <- function(data, response_var, tau_value, use_logit = FALSE) {
  ...
  mod <- tryCatch(
    ...
    error = function(e) { message("LMM failed at τ=", tau_value, ": ", e$message); NULL }
  )
...
# ------------------------------------------------------------
# 5. Fit LMM at every τ level for all three responses
# ------------------------------------------------------------
...
fits_sprayedfrac  <- lapply(tau_grid, fit_one_tau, data = dat,
                            response_var = "sprayed_frac", use_logit = USE_LOGIT)
...
emm_sprayedfrac <- lapply(fits_sprayedfrac, extract_emm_tau) %>% bind_rows()

emm_all <- bind_rows(emm_recall, emm_precision, emm_sprayedfrac)

write.csv(emm_all,
  file.path(out_dir, "NNDM_prevGT0_emm_tau_all_3panel.csv"),
  row.names = FALSE)
cat("Saved EMM table (3-panel).\n")

# ------------------------------------------------------------
# 7. Combined 3-row plot (Recall / Precision / Sprayed fraction) vs τ
# ------------------------------------------------------------
...
emm_all_plot <- emm_all %>%
  mutate(
    response = factor(response,
                      levels = c("recall", "precision", "sprayed_frac"),
                      labels = c("Recall", "Precision", "Sprayed fraction")),
...
ggsave(file.path(out_dir, paste0("NNDM_emm_tau_combined_3panel_", scale_tag, ".png")),
       plot = p_combined, width = 7, height = 9, dpi = 300)
ggsave(file.path(out_dir, paste0("NNDM_emm_tau_combined_3panel_", scale_tag, ".tiff")),
       plot = p_combined, width = 7, height = 9, dpi = 300,
       compression = "lzw")

cat("Saved 3-panel combined τ EMM plot.\n")
