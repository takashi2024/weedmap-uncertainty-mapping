...
Probability analysis for KpR TabPFN under NNDM LOO CV.
Reads  : Results/data_{date}/NNDM/KpR/{target}_test.csv
         Results/data_{date}/NNDM/KpR/{target}_predictions.csv
         Results/data_{date}/NNDM/KpR/{target}_predictions_quantiles.csv
Writes : Results/NNDM_prob/NNDM_KpR_{target}_runs_with_R2_T{T}.csv
         Results/NNDM_prob/NNDM_KpR_{target}_taucurve_T{T}.csv
         Results/NNDM_prob/NNDM_KpR_{target}_detpoint_T{T}.csv
         Results/NNDM_prob/NNDM_KpR_{target}_topb_T{T}.csv
"""
...
# =========================================================
# Helpers (identical logic to original main_prob_analysis_KpR.py)
# =========================================================
...
def infer_q_levels_from_columns(qdf):
    ...
    return np.array(q_levels, dtype=float)
...
def regression_r2_from_run(run_dir, target):
...
def taucurve_from_quantiles(run_dir, target, t_count):
    ...
    p = probs_from_quantile_df(qdf, q_levels=q_levels, y_thresh=y_thresh)

    y_pred_log = pd.read_csv(pred_fp).iloc[:, 0].to_numpy(dtype=float)
    n_hat = np.expm1(y_pred_log)

    if len(p) != len(y_bin) or len(n_hat) != len(y_bin):
        raise ValueError(f"Length mismatch in {run_dir}/{target}")
...
# =========================================================
# Scan NNDM KpR directories
# =========================================================
target_name = list(TARGET_FILTER)[0]

q_files = glob.glob(os.path.join(RESULTS_ROOT, "data_*", "NNDM", "KpR",
                                  f"{target_name}_predictions_quantiles.csv"))
run_dirs = sorted({os.path.dirname(fp) for fp in q_files})
print(f"Found KpR NNDM run dirs for {target_name}: {len(run_dirs)}")
...
for run_dir in run_dirs:
    ...
    r2 = regression_r2_from_run(run_dir, target_name)
    if r2 is None:
    ...
    tau_df, prob_summ, det_point, bcurve_prob, bcurve_det = ret

    run_row = {"date": date, "method": "KpR_NNDM", "target": target_name,
               "run_dir": run_dir, "R2_log_reg": r2, "T_count": float(T_COUNT),
               **prob_summ}

    tau_df = tau_df.copy()
    tau_df["date"] = date; tau_df["method"] = "KpR_NNDM"
    ...
    dp = det_point.copy()
    dp.update({"date": date, "method": "KpR_NNDM", "target": target_name,
               "run_dir": run_dir, "R2_log_reg": r2, "T_count": float(T_COUNT),
    ...
    for bc, stype in [(bcurve_prob, "prob_p"), (bcurve_det, "det_nhat")]:
        b = bc.copy()
        b["date"] = date; b["method"] = "KpR_NNDM"; b["target"] = target_name
    ...
    print(f"  {date}: R2={r2:.4f}  prevalence={prob_summ['prevalence']:.3f}  n={prob_summ['n']}")
...
for df, suffix in [(df_runs, "runs_with_R2"), (df_tau, "taucurve"),
    ...
    if df.empty:
        continue
    out = os.path.join(OUT_DIR, f"NNDM_KpR_{target_name}_{suffix}_T{T_COUNT}.csv")
